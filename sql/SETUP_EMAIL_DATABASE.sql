...
-- Drop existing tables if recreating (CAREFUL: This deletes data!)
...
DROP POLICY IF EXISTS "Users can view their company email logs" ON email_logs;
DROP POLICY IF EXISTS "Users can update their company email logs" ON email_logs;
DROP POLICY IF EXISTS "Users can view system and their company templates" ON email_templates;
DROP POLICY IF EXISTS "Users can insert their company templates" ON email_templates;
DROP POLICY IF EXISTS "Users can update their company templates" ON email_templates;
DROP POLICY IF EXISTS "Users can delete their company templates" ON email_templates;
...
CREATE POLICY "Users can view system and their company templates"
  ...
  USING (is_system = true OR company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid()));
...
CREATE POLICY "Users can update their company templates"
  ...
  USING (is_system = false AND company_id IN (SELECT company_id FROM profiles WHERE id = auth.uid()));
...
-- Drop existing triggers (in case of re-run)
...
-- Delete existing system templates (in case of re-run)
...
SELECT
  ...
  (SELECT count(*) FROM email_templates WHERE is_system = true) as system_templates,
  (SELECT string_agg(name, ', ') FROM email_templates WHERE is_system = true) as template_names;
