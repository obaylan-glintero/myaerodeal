...
const AircraftDetailView = ({ aircraft, closeModal, openModal }) => {
  ...
  const handleViewSpec = () => {
    console.log('📄 Attempting to view spec sheet:', aircraft.specSheet);
    console.log('📄 Spec sheet data available:', !!aircraft.specSheetData);
    console.log('📄 Spec sheet type:', aircraft.specSheetType);

    if (!aircraft.specSheetData) {
      console.error('❌ Spec sheet data is missing');
      alert('Spec sheet data is not available. Please try refreshing the page.');
      return;
    }
    ...
    if (aircraft.specSheetType && aircraft.specSheetType.includes('pdf')) {
      try {
        ...
        if (isMobile) {
          ...
          // Desktop: Create viewer window with iframe (same as AircraftView)
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: colors.secondary, border: `1px solid ${colors.border}` }}>
          <div className="flex items-center justify-between">
            ...
            {aircraft.specSheetData ? (
              ...
              <span className="text-xs italic" style={{ color: colors.textSecondary }}>
                Data not loaded
              </span>
            )}
          </div>
